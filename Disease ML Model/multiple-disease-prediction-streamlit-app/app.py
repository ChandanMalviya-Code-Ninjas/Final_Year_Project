...
heart_scaler = pickle.load(open(f'{working_dir}/saved_models/heart_scaler.sav', 'rb'))
...
if "disease" in query_params:
    disease = query_params["disease"]
    if disease == "heart":
        default_index = 1
    ...
    if st.button('Diabetes Test Result'):
        ...
        if not all([Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction, Age]):
            ...
            try:
                ...
                st.error(f'Invalid input values. Please enter valid numbers. Error: {str(e)}')
    ...
    if st.button('Heart Disease Test Result'):
        ...
        if not all([age, sex, cp, trestbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal]):
            ...
            try:
                ...
                import numpy as np
                user_input_scaled = heart_scaler.transform(np.array(user_input).reshape(1, -1))
